...
class melt_vapor_system:
    ...
    def mass_balance_equation_fastchem(self,fO2,T, volatile_comp):
        ...
        #def residual(logO):
           ...
           # O_abun, T, fO2, P_boa, vapor_partial_pressures, volatile_comp)
        ...
        #have a two case scenario: fsolve if no physical solution, otherwise use brentq
        #if np.sign(f_lo) != np.sign(f_hi):
            # Guaranteed physical root
            #from scipy.optimize import brentq
            #print('physical root')
            #logO_sol = brentq(residual, -12, 12)
            #logO_sol0=logO_sol
        #else:
            #print('careful, no physical root !')
            # Fall back to fsolve (oxygen-excess / saturation regime)
            #logO_sol = fsolve(residual, logO_sol0)[0]
        ...
        for gas in partial_pressures.columns:
            ...
            #This needs to change because fastchem tries to minimise this , but actually this is not what we want. We want that the added oxygen in the atmosphere 
            #agrees what is outgassed and not the total oxygen, so that we can figure out how much silicates are outgassded together with that oxygen ...
    ...
    def calculate_fastchem_abundance(self, O_abun, vapor_partial_pressures, P_boa,\
        ...
        #print('fractional abundance of carbon:', volatile_comp['C'])
        #print('fractional abundance of nitrogen:', volatile_comp['N'])
